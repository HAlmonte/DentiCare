              ...
              <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    <select class="form-control select2" id="patient_id" name="patient_id" required>
                        ...
                        require_once '../database&config/config.php';
                        $cn = new mysqli (HOST, USER, PW, DB);
                        $sql="SELECT patient_id, first_name, middle_name, last_name FROM tbl_patient";
                        $qry=$cn->prepare($sql);
                        $qry->execute();
                        $qry->bind_result($patient_id, $p_first_name, $p_middle_name, $p_last_name);
                        $qry->store_result();
                        while($qry->fetch()){
                            echo "<option value='$patient_id'>$p_first_name $p_middle_name $p_last_name</option>";
                        }
                        ?>
                    </select>
                  </div>
<!--
                  <div class="form-group">
                    ...
                    <input type="text" id='appointment_no' name='appointment_no' class='form-control form-control-border' placeholder="Appointment No." required>
                  </div>  
-->
                  <div class="form-group">
              ...
              <input type="submit" class="btn elevation-1 btn-sm btn-primary " id="add-reservation_btn" name="add-reservation" value="Add Reservation">
              </div>
          </form>
        <!-- /.card-body -->
      </div>
